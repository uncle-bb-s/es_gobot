...
# ================= DATABASE POOL =================
db_pool = psycopg2.pool.ThreadedConnectionPool(1, 10, dsn=DATABASE_URL, cursor_factory=RealDictCursor)
...
async def get_sites_list() -> str:
    ...
    if not rows:
        return "—"
    return "\n".join(f"🌐 <a href='{row['url']}'>{row['name']}</a>" for row in rows)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if WELCOME_IMAGE:
        ...
        await safe_send(update.message.reply_text, caption, parse_mode="HTML")
...
async def link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("INSERT INTO last_requests (user_id, timestamp) VALUES (%s, %s) "
                        "ON CONFLICT (user_id) DO UPDATE SET timestamp = EXCLUDED.timestamp", (user_id, now))
            cur.execute("INSERT INTO active_links (user_id, invite_link, expire) VALUES (%s, %s, %s) "
...
async def addsite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id) or len(context.args) < 2:
        return await safe_send(update.message.reply_text, "❌ Использование: /addsite <название> <ссылка>")
    name = context.args[0]
    url = context.args[1]
    conn = get_db()
    try:
...
async def removesite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id) or not context.args:
        return await safe_send(update.message.reply_text, "❌ Использование: /removesite <название>")
    name = context.args[0]
    conn = get_db()
    try:
        with conn.cursor() as cur:
...
# ===== ADMIN /setchat /addbot /removebot /settings /broadcast =====
